...
Note: The Django app is not started by compose; run it locally with uv/pip, or extend the compose to add the app service.
...
- Dockerfile HEALTHCHECK points to http://localhost:8000/api/v1/health. If your app uses /api/health, update the path or add a versioned prefix in porsche/urls.py.
...
- Exceptions: handled by porsche.core.restframework.views.exception_handler returning unified structure (code/data/message)
